...
function createNewAccount(req, res) {
  ...
  if(!moduleName || !ownerName) {
    console.log("ERROR: A module name and owner name must be specified when creating a new account");
    res.status(400).send({error: "A module name and owner name must be specified when creating a new account"})
  }
  Promise.all([
